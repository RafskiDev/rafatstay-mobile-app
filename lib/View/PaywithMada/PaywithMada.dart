...
import 'PaywithMada_riverpod.dart';
class PaywithMada extends ConsumerWidget {
  PaywithMada({super.key});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    ref.watch(PaywithMada_riverpod);
    final sizes=Sizes(context);
    Themes theme = Themes();
    TextLanguage textLanguage = TextLanguage();
    return  Scaffold(
      appBar:buildCustomAppBar(context,"Pay with Mada"),
          ...
          child:SingleChildScrollView(
              child:Column(
                  children: [
                    ...
                    WidgetTextField(
                      isPassword: false,
                      Controller: ref.read(PaywithMada_riverpod.notifier).cardNumberController,
                      HintText:textLanguage.GetWord("رقم البطاقة"),
                      iconData:"assets/icon/CardNumber.svg",
                      focusNode: ref.read(PaywithMada_riverpod.notifier).cardNumberControllerNode,
                    ),
                    SizedBox(height:sizes.GetHeight()*2,),
                    WidgetTextField(
                      isPassword: false,
                      Controller: ref.read(PaywithMada_riverpod.notifier).cardholderNameController,
                      HintText:textLanguage.GetWord("اسم حامل البطاقة"),
                      iconData:"assets/icon/CardholderName.svg",
                      focusNode: ref.read(PaywithMada_riverpod.notifier).cardholderNameControllerNode,
                    ),
                    SizedBox(height:sizes.GetHeight()*2,),
                    Row(
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: WidgetTextField(
                            isPassword: false,
                            Controller: ref.read(PaywithMada_riverpod.notifier).expiryDateController,
                            HintText:textLanguage.GetWord('تاريخ انتهاء الصلاحية'),
                            iconData:"assets/icon/ExpiryDate.svg",
                            focusNode: ref.read(PaywithMada_riverpod.notifier).expiryDateControllerNode,
                          ),
                        ),
                        SizedBox(
                          width:sizes.GetWidth()*45,
                          child: WidgetTextField(
                            ...
                            isPassword: false,
                            Controller: ref.read(PaywithMada_riverpod.notifier).cVVController,
                            HintText:"CVV",
                            iconData:"assets/icon/cvv.svg",
                            focusNode: ref.read(PaywithMada_riverpod.notifier).cVVControllerNode,
                          ),
                        ),
                      ],
                    ),
                      ...
                      children: [
                        CheckBoxSvg(
                          onChanged:(bool value) {
                            ref.read(PaywithMada_riverpod.notifier).checkBoxSvg(value);
                          },
                          initialValue: false,
                        ),
